...
import type { Material, MaterialVariant, Unit } from '../components/inventory/types'
...
class InventoryDuplicatesErrorBoundary extends Component<{ children: ReactNode }, ErrorBoundaryState> {
      ...
      return (
        <div className="min-h-screen flex items-center justify-center bg-gray-50 p-6">
          <div className="bg-white border border-red-100 rounded-xl shadow-sm p-6 max-w-lg w-full">
            <h1 className="text-lg font-semibold text-red-600">Page failed to load</h1>
            <p className="text-sm text-gray-600 mt-2">{this.state.errorMessage}</p>
            <p className="text-xs text-gray-400 mt-3">Check the browser console for details.</p>
          </div>
        </div>
      )
    }
...
function InventoryDuplicatesPage() {
  ...
  const { data: variants, isLoading, error, refetch, isFetching } = useQuery({
    ...
    queryFn: async () => {
      ...
      const emptyRefs = { orderMaterials: 0, beams: 0, templates: 0, pricing: 0, tags: 0, total: 0 }

      const materialMap = new Map((mats || []).map((m) => [m.id, m as MaterialSummary]))
      ...
      const [opmRes, beamRes, templateRes, pricingRes, tagsRes] = await Promise.all([
        supabase
      ...
      const countBy = <T extends string>(rows: Record<T, string>[] | null, key: T) => {
  ...
  const mergeMutation = useMutation({
    mutationFn: async ({ keepId, deleteIds }: { keepId: string; deleteIds: string[] }) => {
      ...
      const keepTagIds = new Set((keepTags || []).map((row: any) => row.tag_id))
    ...
    onSuccess: async () => {
      setMergeTarget(null)
      setMergeError(null)
      await queryClient.invalidateQueries({ queryKey: ['material-variant-duplicates'] })
    },
    onError: (err: any) => {
        ...
        <main className="flex-1 overflow-y-auto">
          <div className="p-8">
            <div className="flex items-center justify-between mb-8">
              <div>
                <h1 className="text-2xl font-bold text-gray-900">Material Variant Duplicates</h1>
                <p className="text-gray-500 mt-1">Merge duplicates by redirecting references to a single variant.</p>
              </div>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 mb-6">
              <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                <div>
                  <p className="text-sm font-medium text-gray-700">Duplicate groups</p>
                  <p className="text-xs text-gray-500">Grouped by variant name only.</p>
                </div>
            ...
            {isLoading ? (
              ...
              <div className="space-y-6">
                {duplicateGroups.map((group) => {
                  ...
                  return (
                    <div key={group.key} className="bg-white rounded-xl shadow-sm border border-gray-100">
                      <div className="flex flex-col gap-2 px-6 py-4 border-b border-gray-100 md:flex-row md:items-center md:justify-between">
                        <div>
                          <h2 className="text-lg font-semibold text-gray-900">{group.variantName || '—'}</h2>
                          <p className="text-sm text-gray-500">Material: {group.materialName}</p>
                          <p className="text-xs text-gray-400">{group.dimensions} · Unit: {group.unitName}</p>
                        </div>
                        <div className="flex items-center gap-3">
                          <div className="text-xs text-gray-500">
                            {group.variants.length} variants · {totalToMove} references to move
                          </div>
                          <button
                            onClick={() => {
                          ...
                          <tbody>
                            {group.variants.map((variant) => (
                              <tr key={variant.id}>
                                <td>
                                  <label className="flex items-center justify-center">
                                    <input
                                      type="radio"
                                      name={`keep-${group.key}`}
                                      checked={selectionByGroup[group.key] === variant.id}
                                      onChange={() =>
                                        setSelectionByGroup((prev) => ({
                                          ...prev,
                                ...
                                <td className="font-mono text-xs text-gray-600">{shortId(variant.id)}</td>
                                <td className="text-gray-600 max-w-xs truncate">{variant.description || '—'}</td>
                                <td className="text-gray-600">{formatDimensions(variant)}</td>
                                <td>
                                  <span className="px-2 py-1 bg-gray-100 text-gray-700 rounded text-xs">
                                    {variant.unit?.name || '—'}
                                  </span>
                                </td>
                                <td className="text-sm text-gray-500">{formatDate(variant.created_at)}</td>
                                <td className="text-sm text-gray-600 text-center">{variant.refCounts.orderMaterials}</td>
                                <td className="text-sm text-gray-600 text-center">{variant.refCounts.beams}</td>
                                <td className="text-sm text-gray-600 text-center">{variant.refCounts.pricing}</td>
                                <td className="text-sm text-gray-600 text-center">{variant.refCounts.templates}</td>
                                <td className="text-sm text-gray-600 text-center">{variant.refCounts.tags}</td>
                                <td className="text-sm font-semibold text-gray-700 text-center">{variant.refCounts.total}</td>
                              </tr>
                            ))}
                          </tbody>
                        </table>
                      ...
                      {keepVariant && (
                        <div className="px-6 py-4 border-t border-gray-100 text-sm text-gray-600">
                          Keeping: <span className="font-medium text-gray-900">{keepVariant.variant_name || '—'}</span> ({shortId(keepVariant.id)})
                        </div>
                      )}
                    </div>
                  )
          ...
          {mergeTarget && (
            <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
              <div className="bg-white rounded-xl shadow-xl w-full max-w-xl p-6">
                <h3 className="text-lg font-semibold text-gray-900">Merge duplicate variants</h3>
                <p className="text-sm text-gray-600 mt-2">
                  This will update all references to the selected variant and delete the other duplicates.
                </p>
                <div className="mt-4 space-y-2 text-sm text-gray-600">
                  <p>Group: <span className="font-medium text-gray-900">{mergeTarget.variantName || '—'}</span></p>
                  <p>Material: {mergeTarget.materialName}</p>
                  <p>Dimensions: {mergeTarget.dimensions}</p>
                  <p>Unit: {mergeTarget.unitName}</p>
                  <p>
                    Keep variant: <span className="font-medium text-gray-900">{getKeepVariant(mergeTarget)?.variant_name || '—'}</span>
                  </p>
                  <p>
                    Duplicates to delete: <span className="font-medium text-gray-900">{getDeleteIds(mergeTarget).length}</span>
                  </p>
                </div>
                {mergeError && (
                ...
                <div className="flex items-center justify-end gap-3 mt-6">
                  ...
                  <button
                    onClick={() => {
                      const keepId = selectionByGroup[mergeTarget.key] || mergeTarget.variants[0]?.id
...
function formatDimensions(variant: Pick<MaterialVariant, 'length' | 'width' | 'thickness'>) {
