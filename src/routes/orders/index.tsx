...
function OrdersPage() {
  ...
  const filteredOrders = orders?.filter((order: any) => {
    ...
    const matchesStatus = statusFilter === 'all' || order.production_status === statusFilter
  ...
  return (
    <div className="flex h-screen bg-gray-50">
            ...
            {isLoading ? (
              ...
              <>
                <table className="w-full">
                  <thead className="bg-gray-50">
                    <tr>
                      ...
                      <th className="text-left py-3 px-6 text-sm font-medium text-gray-500">Packages</th>
                  ...
                  <tbody>
                    {paginatedOrders.map((order: any) => (
                      ...
                      >
                        ...
                        <td className="py-4 px-6 text-sm text-gray-600">{order.package_count || 0}</td>
                        <td className="py-4 px-6">
                          <span className={`inline-flex px-2.5 py-1 text-xs font-medium rounded-full ${statusColors[order.production_status] || 'bg-gray-100 text-gray-700'}`}>
                ...
                <div className="flex items-center justify-between px-6 py-4 border-t border-gray-100">
                  ...
                  <div className="flex items-center gap-2">
                    <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page === 1} className="p-2 hover:bg-gray-100 rounded-lg disabled:opacity-50">
                      <ChevronLeft className="w-5 h-5" />
                    </button>
                    <span className="text-sm text-gray-600">Page {page} of {totalPages || 1}</span>
                    <button onClick={() => setPage(p => Math.min(totalPages, p + 1))} disabled={page >= totalPages} className="p-2 hover:bg-gray-100 rounded-lg disabled:opacity-50">
      ...
      <OrderCreateDialog open={showCreateDialog} onOpenChange={setShowCreateDialog} />
    </div>
  )
}
