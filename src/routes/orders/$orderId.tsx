...
import { packageItemSchema, packageMaterialSchema, validateInput } from '@/lib/validation'
import {
  ArrowLeft,
  Package,
  Calendar,
...
function OrderDetailPage() {
  ...
  const { data: order, isLoading: orderLoading, error } = useQuery({
    ...
    queryFn: async () => {
      ...
      const unwrapped = {
        ...
        project_lead: Array.isArray(data.project_lead) ? data.project_lead[0] : data.project_lead,
        order_packages: data.order_packages?.map((pkg: any) => ({
          ...pkg,
          original_pkg_info: Array.isArray(pkg.original_pkg_info) ? pkg.original_pkg_info[0] : pkg.original_pkg_info,
          final_pkg_info: Array.isArray(pkg.final_pkg_info) ? pkg.final_pkg_info[0] : pkg.final_pkg_info,
        })) || []
      }
  ...
  const deleteOrderCascade = async () => {
    ...
    try {
      ...
      if (packageIds.length > 0) {
        ...
        const taskLogIds = Array.from(new Set(
          (taskPackages || [])
  ...
  const { data: taskLogs } = useQuery({
    ...
    queryFn: async () => {
      ...
      const taskLogIds = [...new Set(taskPackages.map(tp => tp.task_log_id).filter(Boolean))]
      ...
      return logs.map(log => ({
        ...
        task_assignments: log.task_assignments?.map((a: any) => ({
          ...a,
  ...
  const { data: packageMaterials } = useQuery({
    ...
    queryFn: async () => {
      ...
      const [materialsResult, securingResult] = await Promise.all([materialsPromise, securingPromise])
      ...
      const securingMaterials = securingResult.data.flatMap((securing: any) => {
        ...
        const processBeam = (beam: any, role: string) => {
            ...
            const materialName = Array.isArray(beam.type)
                ? (Array.isArray(beam.type[0]?.material) ? beam.type[0]?.material[0]?.name : beam.type[0]?.material?.name)
                : (Array.isArray(beam.type?.material) ? beam.type?.material[0]?.name : beam.type?.material?.name);

            materials.push({
                id: beam.id,
                order_package_id: securing.order_package_id,
                material_variant_id: Array.isArray(beam.type) ? beam.type[0]?.id : beam.type?.id,
  ...
  const { data: packageManufacturing } = useQuery({
    ...
    queryFn: async () => {
      ...
      data?.forEach((securing: any) => {
        if (securing.securing_template?.horizontal_bar) beamIds.add(securing.securing_template.horizontal_bar)
        if (securing.securing_template?.vertical_bar) beamIds.add(securing.securing_template.vertical_bar)
        if (securing.securing_template?.skids) beamIds.add(securing.securing_template.skids)
      })
      ...
      return data.map((securing: any) => {
        const template = securing.securing_template
        const type = Array.isArray(template?.type) ? template.type[0] : template?.type
        const typeMaterial = Array.isArray(type?.material) ? type.material[0] : type?.material
        ...
        const getBeamData = (beamId: string | null) => {
          ...
          const beamMaterial = Array.isArray(beamType?.material) ? beamType.material[0] : beamType?.material
          return {
            id: beam.id,
  ...
  const { data: packageServices } = useQuery({
    ...
    queryFn: async () => {
      ...
      return data.map((item: any) => ({
        ...item,
        service_name: Array.isArray(item.service) ? item.service[0]?.service : item.service?.service,
      })) as PackageService[]
    },
    enabled: !!user && !!order,
  })
  ...
  const addPackageItemMutation = useMutation({
    mutationFn: async (item: { order_package_id: string; designation: string; quantity: number; length?: number | null; width?: number | null; height?: number | null }) => {
      const { data, error } = await supabase
        .from('package_items')
        .insert(item)
        .select()
  ...
  const updatePackageItemMutation = useMutation({
    mutationFn: async ({ id, ...updates }: { id: string; designation?: string; quantity?: number; length?: number | null; width?: number | null; height?: number | null }) => {
      const { data, error } = await supabase
        .from('package_items')
        .update(updates)
        .eq('id', id)
  ...
  const updatePackageInfoMutation = useMutation({
    mutationFn: async ({ packageId, infoType, updates }: { 
      packageId: string; 
      ...
      const infoId = infoType === 'original' ? pkg.original_pkg_info : pkg.final_pkg_info
      ...
      if (infoId) {
        ...
        const updateField = infoType === 'original' ? 'original_pkg_info' : 'final_pkg_info'
  ...
  const addPackageMaterialMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['packageMaterials', orderId] })
    },
  })
  ...
  const updateOrderStatusMutation = useMutation({
    mutationFn: async ({ production_status, commercial_status }: { production_status?: string; commercial_status?: string }) => {
      const updateData: Record<string, string> = {}
      if (production_status !== undefined) updateData.production_status = production_status
      if (commercial_status !== undefined) updateData.commercial_status = commercial_status
  ...
  const endTaskMutation = useMutation({
    mutationFn: async ({ taskLogId, endTime }: { taskLogId: string; endTime: string }) => {
  ...
  const { data: availableMaterials } = useQuery({
    ...
    queryFn: async () => {
      ...
      return data.map((v: any) => ({
        ...
        material_name: Array.isArray(v.material) ? v.material[0]?.name : v.material?.name,
      }))
    },
    enabled: !!user,
  })
  ...
  const handleAddItem = async () => {
    ...
    setItemForm({ designation: '', quantity: 1, length: '', width: '', height: '' })
  }
  ...
  const calculateProposedChanges = () => {
    ...
    const presentLogs = attendanceLogs.filter(
      log => log.log_date === selectedAttendanceDate && log.status === 'present'
    )
    ...
    for (const log of presentLogs) {
      ...
      if (log.shift_period === 'morning') {
        ...
        if (!log.end_time) {
          ...
          else if (endTime.getHours() > 12 || (endTime.getHours() === 12 && endTime.getMinutes() > 0)) {
            needsChange = true
            newEndTime = correctEndTime
          }
        }
      ...
      if (needsChange && newEndTime) {
        ...
        if (log.start_time) {
          const start = new Date(log.start_time)
          const hours = (newEndTime.getTime() - start.getTime()) / (1000 * 60 * 60)
  ...
  // Approve all changes
  const approveAll = () => {
    setProposedChanges(prev => prev.map(change => ({ ...change, approved: true })))
  }
  ...
  const [selectedPackageTab, setSelectedPackageTab] = useState<'info' | 'items' | 'manufacturing' | 'accessories' | 'securing' | 'services' | 'comments'>('info')

  // Get selected package data
  const selectedPackage = useMemo(() => {
    if (!order?.order_packages || !selectedPackageId) return null
    return order.order_packages.find(pkg => pkg.id === selectedPackageId) || null
  ...
  const selectedPackageItems = useMemo(() => {
    ...
    return packageItems.filter(item => item.order_package_id === selectedPackageId)
  ...
  const selectedPackageManufacturing = useMemo(() => {
    ...
    return packageManufacturing.filter(m => m.order_package_id === selectedPackageId)
  ...
  const selectedPackageMaterials = useMemo(() => {
    if (!packageMaterials || !selectedPackageId) {
      return { accessories: [], securing: [], vacuumPacking: [], gasPacking: [] }
    }
    
    const pkgMaterials = packageMaterials.filter(m => m.order_package_id === selectedPackageId)
    
    return {
      accessories: pkgMaterials.filter(m => m.material_type === 'Accessories'),
      securing: pkgMaterials.filter(m => m.material_type === 'Securing'),
      vacuumPacking: pkgMaterials.filter(m => m.material_type === 'Vacuum Packing'),
  ...
  const selectedPackageServices = useMemo(() => {
    ...
    return packageServices.filter(s => s.order_package_id === selectedPackageId)
  ...
  useEffect(() => {
    if (order?.order_packages?.length && !selectedPackageId) {
      const sortedPackages = [...order.order_packages].sort((a, b) => a.package_number - b.package_number)
  ...
  const filteredAttendance = useMemo(() => {
    if (!attendanceLogs || !selectedAttendanceDate) return []
    const filtered = attendanceLogs.filter(log => log.log_date === selectedAttendanceDate)
    
    // Sort: shift order (morning, afternoon, full_day), then by name A-Z
    const shiftOrder: Record<string, number> = { 'morning': 0, 'afternoon': 1, 'full_day': 2 }
    return filtered.sort((a, b) => {
      const shiftDiff = (shiftOrder[a.shift_period] ?? 99) - (shiftOrder[b.shift_period] ?? 99)
      if (shiftDiff !== 0) return shiftDiff
  ...
  const taskDays = useMemo(() => {
    ...
    const days = [...new Set(tasks.map(log => 
  ...
  const getSmartEndTime = (task: TaskLog): string => {
    ...
    const dayAttendance = attendanceLogs?.filter(log => log.log_date === taskDateStr) || []
    ...
    if (isMorning) {
      ...
      const afternoonLogs = dayAttendance.filter(log => log.shift_period === 'afternoon' && log.end_time)
  ...
  const exportToExcel = async () => {
    ...
    const priceMap: Record<string, { price: number; unit: string; length: number; width: number }> = {}
    
    pricingData?.forEach((p: any) => {
      const variant = Array.isArray(p.material_variant) ? p.material_variant[0] : p.material_variant
      const unitName = Array.isArray(variant?.unit) ? variant?.unit[0]?.name : variant?.unit?.name
    ...
    const getMaterialCost = (mat: PackageMaterial) => {
      ...
      if (matUnit !== priceUnit) {
        ...
        else if (matUnit.toLowerCase() === 'm2' && priceUnit.toLowerCase() === 'roll') {
    ...
    const workDays = [...new Set(attendanceLogs?.map(log => log.log_date) || [])]
    const uniquePackers = [...new Set(attendanceLogs?.map(log => log.packer?.full_name).filter(Boolean) || [])]
    ...
    attendanceLogs?.forEach(log => {
      if (log.start_time && log.end_time && log.packer?.full_name) {
        ...
        totalManHours += hours
        packerHours[log.packer.full_name] = (packerHours[log.packer.full_name] || 0) + hours
      }
    })
    ...
    manpowerSheet.getCell('B6').font = { bold: true, color: { argb: 'FF16A34A' } }

    if (workDays.length > 0) {
      const sortedDays = [...workDays].sort()
      manpowerSheet.getCell('A7').value = 'Start Date:'
      manpowerSheet.getCell('B7').value = new Date(sortedDays[0]).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })
      manpowerSheet.getCell('A8').value = 'End Date:'
      manpowerSheet.getCell('B8').value = new Date(sortedDays[sortedDays.length - 1]).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })
    }
    ...
    Object.entries(packerHours).sort((a, b) => b[1] - a[1]).forEach(([name, hours]) => {
    ...
    const dailyHeaders = ['Date', 'Packer', 'Shift', 'Start Time', 'End Time', 'Hours']
    ...
    sortedAttendance.forEach(log => {
      ...
      const hours = log.start_time && log.end_time
        ? ((new Date(log.end_time).getTime() - new Date(log.start_time).getTime()) / (1000 * 60 * 60)).toFixed(1)
        : '—'
      ...
      row.values = [
        ...
        log.shift_period,
        log.start_time ? new Date(log.start_time).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' }) : '—',
    ...
    attendanceSheet.columns = [
      { width: 15 }, { width: 25 }, { width: 15 }, { width: 15 }, { width: 15 }, { width: 12 }
    ]
    ...
    const materialSummary: Record<string, { qty: number; unit: string; cost: number }> = {}
    ...
    Object.entries(materialSummary).sort((a, b) => a[0].localeCompare(b[0])).forEach(([name, data]) => {
      const row = matSummarySheet.getRow(rowIndex)
      row.values = [name, data.qty, data.unit, data.cost > 0 ? `AED ${data.cost.toFixed(2)}` : '—']
    ...
    matSummarySheet.columns = [{ width: 40 }, { width: 15 }, { width: 10 }, { width: 20 }]
    ...
    rowIndex = 3
    const packages = [...(order.order_packages || [])].sort((a, b) => a.package_number - b.package_number)
    ...
    packages.forEach(pkg => {
      ...
      pkgHeaderCell.font = { bold: true, size: 12, color: { argb: 'FFFFFFFF' } }
      pkgHeaderCell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF1F2937' } }
      rowIndex++

      // Materials headers
      const matHeaders = ['Category', 'Material', 'Variant', 'Qty', 'Dimensions', 'Unit', 'Est. Cost (AED)', 'Comment']
        ...
        types.forEach(type => {
          ...
          if (mats && mats.length > 0) {
            mats.forEach((mat, idx) => {
              ...
              const dims = mat.length && mat.width && mat.height 
      ...
      // Others
      addMaterialRows(['Accessories', 'Vacuum Packing', 'Gas Packing', 'Securing'])
    ...
    matBreakdownSheet.columns = [
      { width: 18 }, { width: 25 }, { width: 20 }, { width: 10 }, { width: 20 }, { width: 10 }, { width: 20 }, { width: 30 }
    ]
        ...
        <main className="flex-1 overflow-y-auto">
          <div className="p-6">
            ...
            <div className="bg-red-50 border border-red-200 rounded-lg p-6 text-center">
              ...
              <Link to="/orders" className="inline-block px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">
                Back to Orders
              </Link>
            </div>
          </div>
            ...
            <div className="lg:col-span-2 space-y-6">
                  ...
                  <div className="flex items-center gap-3">
                    <div className="flex items-center gap-2">
                      ...
                      <select
                        ...
                        onChange={(e) => updateOrderStatusMutation.mutate({ production_status: e.target.value })}
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <select
                        ...
                        onChange={(e) => updateOrderStatusMutation.mutate({ commercial_status: e.target.value })}
                        disabled={updateOrderStatusMutation.isPending}
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                  ...
                  <div className="text-center p-3 bg-gray-50 rounded-lg">
                    <p className="text-2xl font-bold text-green-600">
                      {order.order_packages?.filter(p => p.status === 'packed' || p.status === 'delivered').length || 0}
                    </p>
                    <p className="text-sm text-gray-500">Completed</p>
                  </div>
                  <div className="text-center p-3 bg-gray-50 rounded-lg">
                    <p className="text-2xl font-bold text-blue-600">
                      {order.order_packages?.filter(p => p.status === 'in_production').length || 0}
                    </p>
                    <p className="text-sm text-gray-500">In Production</p>
                  </div>
                  <div className="text-center p-3 bg-gray-50 rounded-lg">
                    <p className="text-2xl font-bold text-amber-600">
                      {order.order_packages?.filter(p => !p.status || p.status === 'design' || p.status === 'approved').length || 0}
                    </p>
                    ...
                    <table className="excel-table">
                      ...
                      <tbody>
                        {order.order_packages
                          ...
                          .map((pkg) => (
                            <tr key={pkg.id}>
                              <td className="font-medium">{pkg.package_number}</td>
                              <td className="text-gray-600 max-w-xs truncate">
                                {pkg.description || '—'}
                              </td>
                              <td>{getDimensions(pkg) || '—'}</td>
                              <td>{getWeight(pkg) ? `${getWeight(pkg)} kg` : '—'}</td>
                              <td>
                                <select
                                  value={pkg.status || 'pending'}
                                  onChange={(e) => updatePackageStatusMutation.mutate({ 
                                    packageId: pkg.id, 
              ...
              <div className="bg-white rounded-lg border shadow-sm overflow-hidden">
                <div className="px-6 py-4 border-b flex items-center gap-2">
                  ...
                  <span className="ml-auto text-sm text-gray-500">{teamMembers?.length || 0} packers</span>
                </div>
                      ...
                      .map((member) => (
                        ...
                        >
                          <div className={`w-10 h-10 rounded-full flex items-center justify-center ${
                            member.is_team_lead ? 'bg-blue-500' : 'bg-gray-400'
                          }`}>
              ...
              <div className="bg-white rounded-lg border shadow-sm overflow-hidden">
                <div className="px-6 py-4 border-b flex items-center gap-2">
                  ...
                  <h2 className="text-lg font-semibold text-gray-900">Attendance & Work Sessions</h2>
                  <div className="ml-auto flex items-center gap-3">
                    <span className="text-sm text-gray-600">{attendanceDates.length} days • {attendanceLogs?.length || 0} records</span>
                    ...
                    <div className="border-b bg-gray-50 px-4 py-2 overflow-x-auto">
                      <div className="flex gap-1 min-w-max">
                        {attendanceDates.map((date) => {
                          const dayLogs = attendanceLogs?.filter(l => l.log_date === date) || []
                            ...
                            >
                              ...
                              {hasProjectStart && (
                                <span className={`ml-1.5 text-xs px-1 py-0.5 rounded ${
                                  selectedAttendanceDate === date ? 'bg-white/20' : 'bg-purple-100 text-purple-700'
                                }`}>
                                  Start
                                </span>
                              )}
                              <span className={`ml-1.5 text-xs ${selectedAttendanceDate === date ? 'text-blue-200' : 'text-gray-400'}`}>
                                ({dayLogs.length})
                              </span>
                            </button>
                          )
                              ...
                              <tr key={log.id}>
                                ...
                                <td>
                                  <div className="flex items-center gap-1.5">
                                    ...
                                    <span className="text-gray-700">{getShiftLabel(log.shift_period)}</span>
                                  </div>
                                </td>
                                <td className="text-gray-700">{formatTime(log.start_time)}</td>
                                <td className="text-gray-700">{formatTime(log.end_time)}</td>
                                <td className="text-gray-700">{hours ? `${hours}h` : '—'}</td>
                                <td>
                                  <span
              ...
              <div className="bg-white rounded-lg border shadow-sm overflow-hidden">
                <div className="px-6 py-4 border-b flex items-center gap-2">
                  ...
                  <h2 className="text-lg font-semibold text-gray-900">Tasks by Package</h2>
                  <span className="ml-auto text-sm text-gray-600">{taskLogs?.length || 0} task sessions</span>
                </div>
                  ...
                  <>
                    ...
                    <div className="border-b bg-gray-50 px-4 py-2 overflow-x-auto">
                      <div className="flex gap-1 min-w-max">
                        {[...order.order_packages]
                          .sort((a, b) => a.package_number - b.package_number)
                          .map((pkg) => {
                            const pkgTasks = taskLogs?.filter(log => 
                              log.task_packages.some(tp => tp.order_package_id === pkg.id)
                            ) || []
                            return (
                              <button
                                key={pkg.id}
                              ...
                              >
                                ...
                                <span className={`ml-1.5 text-xs ${selectedPackageId === pkg.id ? 'text-blue-200' : 'text-gray-400'}`}>
                                  ({pkgTasks.length})
                                </span>
                              </button>
                            )
                    ...
                    {taskDays.length > 0 && (
                      <div className="border-b bg-white px-4 py-2 overflow-x-auto">
                        <div className="flex gap-1 min-w-max">
                          ...
                          >
                            All Days ({taskLogs?.filter(log => log.task_packages.some(tp => tp.order_package_id === selectedPackageId)).length || 0})
                          </button>
                          {taskDays.map((day) => {
                            const dayTasks = taskLogs?.filter(log => 
                              log.task_packages.some(tp => tp.order_package_id === selectedPackageId) &&
                              new Date(log.start_time).toISOString().split('T')[0] === day
                            ) || []
                            return (
                              <button
                                key={day}
                              ...
                              >
                                ...
                                <span className="ml-1 opacity-70">({dayTasks.length})</span>
                              </button>
                            )
                          })}
                        </div>
                    ...
                    {tasksForPackage.length > 0 ? (
                        ...
                        <table className="excel-table">
                          ...
                          <tbody>
                            {tasksForPackage.map((log) => {
                              const packerCount = log.task_assignments.filter(a => a.packer?.full_name).length || 1
                              const totalMinutes = log.duration_minutes ? log.duration_minutes * packerCount : null
                              return (
                                <tr key={log.id}>
                                  <td className="font-medium text-gray-900">
                                    <div className="flex items-center gap-2">
                                  ...
                                  <td className="text-gray-700 max-w-[200px]">
                                    <span className="line-clamp-2">{log.task_assignments.map(a => a.packer?.full_name).filter(Boolean).join(', ') || '—'}</span>
                                  </td>
                                  ...
                                  <td className="text-gray-700 whitespace-nowrap">
                                    {log.end_time 
                                      ...
                                      : <span className="text-blue-600 flex items-center gap-1"><Timer className="w-3 h-3" /> In Progress</span>
                                    }
                                  </td>
                                  ...
                                  <td className="text-gray-900 font-medium">
                                    ...
                                    {packerCount > 1 && totalMinutes && (
                                      <span className="text-xs text-gray-400 ml-1">({packerCount}×)</span>
                                    )}
                                  </td>
                                  ...
                                  <td>
                                    {!log.end_time && (
                                      <button
                                        onClick={() => handleOpenEndTaskModal(log)}
                      ...
                      <div className="p-6 text-center text-gray-500">
                        ...
                        <p>No tasks recorded for this {selectedTaskDay !== 'all' ? 'day' : 'package'}</p>
                      </div>
                    )}
                  </>
                ) : (
                  ...
                  <>
                    ...
                    <div className="border-b overflow-x-auto">
                      <div className="flex p-2 gap-1 min-w-max">
                        {[...order.order_packages].sort((a, b) => a.package_number - b.package_number).map((pkg) => (
                          <button
                            key={pkg.id}
                            onClick={() => {
                              setSelectedPackageId(pkg.id)
                      ...
                      <>
                        ...
                        <div className="border-b bg-gray-50">
                          <div className="flex p-2 gap-1 overflow-x-auto">
                            {[
                              { key: 'info', label: 'Info' },
                              { key: 'items', label: 'Items', count: selectedPackageItems.length },
                              { key: 'manufacturing', label: 'Manufacturing', count: selectedPackageManufacturing.length },
                              { key: 'accessories', label: 'Accessories', count: selectedPackageMaterials.accessories.length },
                              { key: 'securing', label: 'Securing', count: selectedPackageMaterials.securing.length },
                              { key: 'services', label: 'Services', count: selectedPackageServices.length },
                              { key: 'comments', label: 'Comments', count: (selectedPackage.comments || []).length },
                            ].map((tab) => (
                              <button
                                key={tab.key}
                                onClick={() => setSelectedPackageTab(tab.key as any)}
                              ...
                              >
                                ...
                                {tab.count !== undefined && (
                                  <span className={`ml-1 text-xs ${
                                    selectedPackageTab === tab.key ? 'text-gray-500' : 'text-gray-400'
                                  }`}>
                                    ({tab.count})
                                  </span>
                                )}
                        ...
                        <div className="p-6">
                          ...
                          {selectedPackageTab === 'info' && (
                            <PackageInfoTab
                              ...
                              updatePackageStatusMutation={updatePackageStatusMutation}
                          ...
                          {/* Manufacturing Tab */}
                          {selectedPackageTab === 'manufacturing' && selectedPackageManufacturing && (
                            <ManufacturingTab
                              selectedPackageManufacturing={selectedPackageManufacturing}
                            />
                          )}
                          ...
                          {selectedPackageTab === 'accessories' && (
                            <AccessoriesTab
                              ...
                              updatePackageMaterialMutation={updatePackageMaterialMutation}
                              deletePackageMaterialMutation={deletePackageMaterialMutation}
              ...
              {/* Add Item Modal */}
              <Dialog.Root open={showAddItemModal} onOpenChange={setShowAddItemModal}>
                <Dialog.Portal>
                  ...
                  <Dialog.Content className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-50 w-full max-w-md bg-white rounded-xl shadow-2xl p-6">
                    <Dialog.Title className="text-lg font-semibold text-gray-900 mb-4">Add Package Item</Dialog.Title>
                    <Dialog.Description className="sr-only">Add a new item to this package</Dialog.Description>
                    
                    <div className="space-y-4">
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">Item Name / Designation</label>
                        <input
                          type="text"
                          value={itemForm.designation}
                          onChange={(e) => {
                            setItemForm(f => ({ ...f, designation: e.target.value }))
                            setItemValidationErrors(prev => ({ ...prev, designation: '' }))
                          }}
                          className={`w-full px-3 py-2 border rounded-lg ${
                            itemValidationErrors.designation ? 'border-red-500 focus:ring-red-500' : ''
                          }`}
                          placeholder="Enter item name"
                        />
                        {itemValidationErrors.designation && (
                          <p className="mt-1 text-sm text-red-600">{itemValidationErrors.designation}</p>
                        )}
                      </div>
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            ...
                            setItemValidationErrors(prev => ({ ...prev, quantity: '' }))
                          }}
                          className={`w-full px-3 py-2 border rounded-lg ${
                            itemValidationErrors.quantity ? 'border-red-500 focus:ring-red-500' : ''
                          }`}
                          min={1}
                        />
                        {itemValidationErrors.quantity && (
                          <p className="mt-1 text-sm text-red-600">{itemValidationErrors.quantity}</p>
                        )}
                      </div>
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">Dimensions (L × W × H) - Optional</label>
                          ...
                          <div>
                            <input 
                              ...
                              onChange={(e) => {
                                ...
                                setItemValidationErrors(prev => ({ ...prev, length: '' }))
                              }}
                              className={`px-3 py-2 border rounded-lg w-full ${
                                itemValidationErrors.length ? 'border-red-500' : ''
                              }`}
                            />
                            {itemValidationErrors.length && (
                              <p className="mt-0.5 text-xs text-red-600">{itemValidationErrors.length}</p>
                            )}
                          </div>
                          <div>
                            <input 
                              ...
                              onChange={(e) => {
                                ...
                                setItemValidationErrors(prev => ({ ...prev, width: '' }))
                              }}
                              className={`px-3 py-2 border rounded-lg w-full ${
                                itemValidationErrors.width ? 'border-red-500' : ''
                              }`}
                            />
                            {itemValidationErrors.width && (
                              <p className="mt-0.5 text-xs text-red-600">{itemValidationErrors.width}</p>
                            )}
                          </div>
                          <div>
                            <input 
                              ...
                              onChange={(e) => {
                                ...
                                setItemValidationErrors(prev => ({ ...prev, height: '' }))
                              }}
                              className={`px-3 py-2 border rounded-lg w-full ${
                                itemValidationErrors.height ? 'border-red-500' : ''
                              }`}
                            />
                            {itemValidationErrors.height && (
                              <p className="mt-0.5 text-xs text-red-600">{itemValidationErrors.height}</p>
                            )}
                          </div>
                        </div>
                      </div>
                    ...
                    <div className="flex justify-end gap-2 mt-6">
                      ...
                      >
                        {addPackageItemMutation.isPending && <Loader2 className="w-4 h-4 animate-spin" />}
                        Add Item
                      </button>
                    </div>
                  </Dialog.Content>
              ...
              {/* Add Material Modal */}
              <Dialog.Root open={showAddMaterialModal} onOpenChange={setShowAddMaterialModal}>
                <Dialog.Portal>
                  <Dialog.Overlay className="fixed inset-0 bg-black/50 z-50" />
                  <Dialog.Content className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-50 w-full max-w-lg bg-white rounded-xl shadow-2xl p-6">
                    <Dialog.Title className="text-lg font-semibold text-gray-900 mb-4">
                      Add {materialType === 'Accessories' ? 'Accessory' : 'Manufacturing Material'}
                    </Dialog.Title>
                    <Dialog.Description className="sr-only">Add a new material to this package</Dialog.Description>
                    ...
                    <div className="space-y-4">
                      ...
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">Material Variant</label>
                        <select
                          value={materialForm.material_variant_id}
                          onChange={(e) => {
                            setMaterialForm(f => ({ ...f, material_variant_id: e.target.value }))
                            setMaterialValidationErrors(prev => ({ ...prev, material_variant_id: '' }))
                          }}
                          className={`w-full px-3 py-2 border rounded-lg ${
                            materialValidationErrors.material_variant_id ? 'border-red-500 focus:ring-red-500' : ''
                          }`}
                        >
                          <option value="">Select a material...</option>
                          {availableMaterials?.map((m) => (
                            <option key={m.id} value={m.id}>
                              {m.variant_name} {m.material_name ? `(${m.material_name})` : ''}
                            </option>
                          ))}
                        </select>
                        {materialValidationErrors.material_variant_id && (
                          <p className="mt-1 text-sm text-red-600">{materialValidationErrors.material_variant_id}</p>
                        )}
                      </div>
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          ...
                          <input
                            ...
                            onChange={(e) => {
                              setMaterialForm(f => ({ ...f, quantity: Number(e.target.value) }))
                              setMaterialValidationErrors(prev => ({ ...prev, quantity: '' }))
                            }}
                            className={`w-full px-3 py-2 border rounded-lg ${
                              materialValidationErrors.quantity ? 'border-red-500 focus:ring-red-500' : ''
                            }`}
                            min={1}
                          />
                          {materialValidationErrors.quantity && (
                            <p className="mt-1 text-sm text-red-600">{materialValidationErrors.quantity}</p>
                          )}
                        </div>
                        <div>
                      ...
                      <div>
                        ...
                        <div className="grid grid-cols-3 gap-2">
                          <input type="number" placeholder="Length" value={materialForm.length} onChange={(e) => setMaterialForm(f => ({ ...f, length: e.target.value }))} className="px-3 py-2 border rounded-lg" />
                          <input type="number" placeholder="Width" value={materialForm.width} onChange={(e) => setMaterialForm(f => ({ ...f, width: e.target.value }))} className="px-3 py-2 border rounded-lg" />
                          <input type="number" placeholder="Height" value={materialForm.height} onChange={(e) => setMaterialForm(f => ({ ...f, height: e.target.value }))} className="px-3 py-2 border rounded-lg" />
                        </div>
                      </div>
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            setMaterialForm(f => ({ ...f, comment: e.target.value }))
                            setMaterialValidationErrors(prev => ({ ...prev, comment: '' }))
                          }}
                          className={`w-full px-3 py-2 border rounded-lg ${
                            materialValidationErrors.comment ? 'border-red-500 focus:ring-red-500' : ''
                          }`}
                          placeholder="Optional comment (max 2000 chars)"
                        />
                        {materialValidationErrors.comment && (
                          <p className="mt-1 text-sm text-red-600">{materialValidationErrors.comment}</p>
                        )}
                      </div>
                      <div className="flex items-center gap-2">
                        <input
                          ...
                          onChange={(e) => setMaterialForm(f => ({ ...f, is_final: e.target.checked }))}
                          className="rounded"
                        />
                        <label htmlFor="is_final" className="text-sm text-gray-700">Mark as Final</label>
                      </div>
                    </div>
                    ...
                    <div className="flex justify-end gap-2 mt-6">
                      ...
                      <button
                        ...
                        className="flex items-center gap-2 px-4 py-2 text-sm bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50"
                      >
                        {addPackageMaterialMutation.isPending && <Loader2 className="w-4 h-4 animate-spin" />}
                        Add Material
                      </button>
                    </div>
                  </Dialog.Content>
              ...
              {/* End Task Modal */}
              <Dialog.Root open={showEndTaskModal} onOpenChange={setShowEndTaskModal}>
                <Dialog.Portal>
                  <Dialog.Overlay className="fixed inset-0 bg-black/50 z-50" />
                  <Dialog.Content className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-50 w-full max-w-md bg-white rounded-xl shadow-2xl p-6">
                    <Dialog.Title className="text-lg font-semibold text-gray-900 mb-2">End Task</Dialog.Title>
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="bg-gray-50 rounded-lg p-4">
                          <div className="flex items-center gap-2 mb-2">
                            <Play className="w-4 h-4 text-blue-500" />
                            <span className="font-medium text-gray-900">{endingTask.task?.name || 'Unknown Task'}</span>
                          </div>
                          <div className="text-sm text-gray-600">
                            <p><strong>Started:</strong> {new Date(endingTask.start_time).toLocaleString('en-US', {
                              month: 'short',
                              day: 'numeric',
                              year: 'numeric',
                              hour: '2-digit',
                            ...
                            <p><strong>Packers:</strong> {endingTask.task_assignments.map(a => a.packer?.full_name).filter(Boolean).join(', ') || 'None assigned'}</p>
                          </div>
                        </div>
                        ...
                        {endTaskTime && (
                          <div className="bg-blue-50 rounded-lg p-3">
                            <p className="text-sm text-blue-700">
                              ...
                              {endingTask.task_assignments.filter(a => a.packer?.full_name).length > 1 && (
                                <span className="ml-2">
                                  (Total: {(() => {
                                    const start = new Date(endingTask.start_time)
                                    const end = new Date(endTaskTime)
                                    const minutes = Math.round((end.getTime() - start.getTime()) / (1000 * 60))
                                    const packerCount = endingTask.task_assignments.filter(a => a.packer?.full_name).length
                    ...
                    <div className="flex justify-end gap-2 mt-6">
                      ...
                      >
                        {endTaskMutation.isPending && <Loader2 className="w-4 h-4 animate-spin" />}
            ...
            <div className="space-y-6">
                ...
                <div className="space-y-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="font-medium text-gray-900">{order.clients?.name || 'N/A'}</p>
                    </div>
                  </div>
                  {order.clients?.contact_person && (
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <p className="font-medium text-gray-900">{order.clients.contact_person}</p>
                      </div>
                    </div>
                  )}
                  {order.clients?.email && (
                  ...
                  {order.clients?.address && (
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <p className="font-medium text-gray-900">{order.clients.address}</p>
                      </div>
                    </div>
                  )}
                </div>
                ...
                <div className="bg-white rounded-lg border shadow-sm p-6">
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="font-medium text-gray-900">{teamMembers?.length || 0} packers</span>
                    </div>
                    <div className="flex items-center justify-between">
                      <span className="text-gray-600">Work Sessions</span>
                      <span className="font-medium text-gray-900">{attendanceLogs?.length || 0}</span>
                    </div>
                    <div className="flex items-center justify-between">
              ...
              <div className="bg-white rounded-lg border border-red-200 shadow-sm p-6">
                ...
                <p className="text-sm text-gray-600 mb-4">
                  Deleting this order removes all related records, including packages, materials, tasks, and manufacturing data.
                </p>
                  ...
                  <Dialog.Portal>
                    ...
                    <Dialog.Content className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white rounded-xl shadow-2xl z-50 w-full max-w-2xl p-6">
                      ...
                      <Dialog.Description className="text-sm text-gray-500 mt-1">
                        This action is permanent. The following tables will be cleaned up for this order.
                      </Dialog.Description>

                      <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-2">
                        {deleteOrderTargets.map((target) => (
                          <div key={target} className="flex items-center gap-2 rounded-lg border border-gray-200 px-3 py-2 text-sm text-gray-700">
          ...
          <Dialog.Content className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white rounded-xl shadow-2xl z-50 w-full max-w-4xl max-h-[85vh] overflow-hidden flex flex-col">
            ...
            <div className="flex-1 overflow-auto p-6">
              {proposedChanges.length > 0 ? (
                <>
                  <p className="text-sm text-gray-600 mb-4">
                    The following attendance records have issues (missing end times, end times on wrong day, negative hours, or invalid times). 
                    Morning shifts will be corrected to 12:00 PM, afternoon shifts to 11:59 PM of the start date.
                  </p>
                  <div className="overflow-x-auto border rounded-lg">
                    <table className="w-full text-sm">
                      <thead className="bg-gray-50 text-left">
                        <tr>
                          <th className="px-4 py-3 font-semibold text-gray-700">Packer</th>
                          <th className="px-4 py-3 font-semibold text-gray-700">Shift</th>
                          <th className="px-4 py-3 font-semibold text-gray-700">Start</th>
                          <th className="px-4 py-3 font-semibold text-gray-700">Current End</th>
                          <th className="px-4 py-3 font-semibold text-gray-700">Hours</th>
                          <th className="px-4 py-3 font-semibold text-green-700">New End</th>
                          <th className="px-4 py-3 font-semibold text-green-700">New Hours</th>
                          <th className="px-4 py-3 font-semibold text-gray-700 text-center">Approve</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y">
                        {proposedChanges.map((change) => (
                          <tr key={change.id} className={change.approved ? 'bg-green-50' : ''}>
                            <td className="px-4 py-3 font-medium text-gray-900">{change.packerName}</td>
                            <td className="px-4 py-3 text-gray-700 capitalize">{change.shift}</td>
                            <td className="px-4 py-3 text-gray-700">
                              {change.currentStart 
                                ? new Date(change.currentStart).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' })
                                : '—'
                              }
                            </td>
                            <td className="px-4 py-3 text-gray-700">
                              {change.currentEnd ? (
                                <span className="text-red-600">
                                  {new Date(change.currentEnd).toLocaleString('en-US', { 
                                    month: 'short', 
                                    day: 'numeric',
                                    hour: '2-digit', 
                                    minute: '2-digit' 
                            ...
                            <td className="px-4 py-3">
                              <span className={parseFloat(change.currentHours) > 12 ? 'text-red-600 font-semibold' : 'text-gray-700'}>
                                {change.currentHours}
                              </span>
                            </td>
                            <td className="px-4 py-3 text-green-700 font-medium">
                              {new Date(change.newEnd).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' })}
                            </td>
                            <td className="px-4 py-3 text-green-700 font-medium">{change.newHours}</td>
                ...
                <div className="text-center py-12">
                  ...
                  <p className="text-gray-700 font-medium">All morning attendance records look good!</p>
                  <p className="text-gray-500 text-sm mt-1">No records need cleaning for the selected date.</p>
                </div>
              )}
            </div>
            ...
            {proposedChanges.length > 0 && (
              <div className="px-6 py-4 border-t bg-gray-50 flex items-center justify-between">
                <div className="text-sm text-gray-600">
                  {proposedChanges.filter(c => c.approved).length} of {proposedChanges.length} records selected
                </div>
                <div className="flex items-center gap-3">
                  <button
                    onClick={approveAll}
                  ...
                  <button
                    ...
                    disabled={applyingChanges || proposedChanges.filter(c => c.approved).length === 0}
                  ...
                  >
                    {applyingChanges ? (
                      ...
                      <>
                        <Check className="w-4 h-4" />
                        Apply Selected ({proposedChanges.filter(c => c.approved).length})
                      </>
                    )}
                  </button>
                </div>
