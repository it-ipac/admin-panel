								...
								return (
									...
									>
										...
										{(() => {
											...
											if (!pkg.show_box_photos || inst.is_continuation || visibleBoxPhotos.length === 0) return null;
											return (
												<div
													style={{
														display: "flex",
															...
															{sortedItems(inst.pkd_items).map((item, idx) => (
																<React.Fragment key={item.id}>
																	...
																	{(() => {
																		...
																		if (!pkg.show_item_photos || visibleItemPhotos.length === 0) return null;
																		return (
																			<tr
																				style={{
																					background:
																			...
																			>
																				<td colSpan={10} style={{ padding: "4px 8px 6px 30px" }}>
																					<div style={{ display: "flex", flexWrap: "wrap", gap: "6px" }}>
																						{visibleItemPhotos.map((url, uidx) => (
																							<img
																								key={`item-photo-${uidx}`}
																								src={url}
																								alt={`Item photo ${uidx + 1}`}
																								style={{
																									height: "45px",
																									objectFit: "cover",
																									borderRadius: "3px",
