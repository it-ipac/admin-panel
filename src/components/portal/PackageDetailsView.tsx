				...
				<>
					<div className="space-y-4 border-y border-[var(--package-border-soft)] bg-[var(--package-canvas-bg)] p-3 sm:p-5">
						{visibleItems.map((item, itemIndex) => {
							...
							const itemReference =
								item.reference && item.reference !== itemTitle ? item.reference : null;
							return (
								<article
									key={item.id}
									...
									<div className="flex items-start justify-between gap-4">
										<div className="min-w-0">
											<p className="text-[10px] font-bold uppercase tracking-[0.15em] text-app-text-muted">
												{item.itemNumber ? "Item #" : "Item reference"}
											</p>
											{item.itemId ? (
												<Link
													to="/portal/item/$id"
											...
											{itemReference && (
												<p className="mt-0.5 break-all text-[11px] font-medium text-app-text-muted">
