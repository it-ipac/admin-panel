...
export function VariantsTable({ variants, onEditVariant, onDeleteVariant, formatDate }: VariantsTableProps) {
  return (
    <table className="excel-table">
      ...
      <tbody>
        {variants.map((variant) => {
          ...
          const firstPricing = hasPricing ? variant.supplier_pricing![0] : null

          return (
            <tr key={variant.id}>
              <td className="font-medium text-gray-900">{variant.variant_name}</td>
              <td>
              ...
              <td>
                <div className="flex flex-wrap gap-1">
                  {variant.material_variant_tags && variant.material_variant_tags.length > 0 ? (
              ...
              <td className="text-right">
                <span className="font-medium text-gray-700">
                  {firstPricing?.supplier_quantity != null ? firstPricing.supplier_quantity : '—'}
                </span>
              </td>
              <td className="text-right">
                <span className="font-medium text-green-600">
                  {firstPricing?.price != null ? `AED ${firstPricing.price.toFixed(2)}` : '—'}
                </span>
              </td>
              <td className="text-right">
                <span className="text-gray-600">
                  {firstPricing?.price_per_unit != null ? `AED ${firstPricing.price_per_unit.toFixed(2)}` : '—'}
                </span>
              </td>
