...
import { Camera, Package, Image, ExternalLink, Download, X } from 'lucide-react'
...
const sectionOrder = ['empty_crate', 'items_placed', 'half_packed', 'fully_packed', 'secured', 'closed_ready', 'other']

export function MediaGallery({ mediaItems, orderPackages }: MediaGalleryProps) {
  const [selectedMediaPackage, setSelectedMediaPackage] = useState<string | null>(null)
  const [selectedMediaCategory, setSelectedMediaCategory] = useState<string>('empty_crate')
  ...
  const mediaByPackage = useMemo((): MediaByPackage[] => {
    ...
    const result = orderPackages
      ...
      .map(pkg => {
        ...
        const grouped = pkgMedia.reduce((acc, item) => {
  ...
  const selectedPackageDesignations = useMemo(() => {
    ...
    const pkg = mediaByPackage.find(p => p.packageId === selectedMediaPackage)
  ...
  const selectedCategoryMedia = useMemo(() => {
    if (!selectedMediaCategory || selectedPackageDesignations.length === 0) return []
    const category = selectedPackageDesignations.find(c => c.key === selectedMediaCategory)
    ...
    <>
      <div className="bg-white rounded-lg border shadow-sm overflow-hidden">
        <div className="px-6 py-4 border-b flex items-center gap-2">
          ...
          <span className="ml-auto text-sm text-gray-600">{mediaItems?.length || 0} images</span>
        </div>
                      ...
                      >
                        {imageUrl ? (
                          <>
                            <img
                              src={imageUrl}
                              alt={media.notes || selectedMediaCategory || 'Image'}
                            ...
                            <div className="absolute inset-0 bg-linear-to-t from-black/50 to-transparent opacity-0 group-hover:opacity-100 transition-opacity">
                              <div className="absolute bottom-0 left-0 right-0 p-2">
                                {media.notes && (
                                  <p className="text-white text-xs truncate">{media.notes}</p>
                                )}
                            ...
                            <button
                              onClick={(e) => {
                                ...
                                link.href = imageUrl
                                link.download = media.image_url?.split('/').pop() || 'image.jpg'
                                link.click()
                              }}
          ...
          <Dialog.Content className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-50 max-w-4xl w-[90vw] max-h-[90vh] bg-white rounded-xl shadow-2xl overflow-hidden">
            ...
            {previewImage && (
              <div className="flex flex-col">
                {/* Image */}
                <div className="relative bg-gray-900 flex items-center justify-center min-h-[300px] max-h-[70vh]">
                  <img
                ...
                <div className="p-4 border-t bg-gray-50 flex items-center justify-between">
                  <div>
                    {previewImage.notes && (
                      <p className="text-sm text-gray-700 font-medium">{previewImage.notes}</p>
                    )}
                    ...
                    <button
                      onClick={() => {
                        ...
                        if (url) {
                          const link = document.createElement('a')
                          link.href = url
                          link.download = previewImage.image_url?.split('/').pop() || 'image.jpg'
                          link.click()
                        }
                      }}
