...
import { Plus, Edit, Trash2, Check, X, Loader2, Sparkles, Camera } from 'lucide-react'
    ...
    <div className="space-y-4">
      ...
      {selectedPackageMaterials.accessories.length > 0 ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm text-left border-collapse">
              <thead className="bg-gray-100 text-gray-700 uppercase text-xs font-bold border-b-2 border-gray-300">
                <tr>
                  <th className="py-3 px-4 border-r-2 border-gray-300">Accessory</th>
                  <th className="py-3 px-4 text-center border-r-2 border-gray-300">Qty</th>
                  <th className="py-3 px-4 text-center border-r-2 border-gray-300">Dimensions</th>
                  <th className="py-3 px-4 text-center border-r-2 border-gray-300">Status</th>
                  <th className="py-3 px-4 border-r-2 border-gray-300">Comment</th>
                  <th className="py-3 px-4 text-center">Actions</th>
                </tr>
              </thead>
              <tbody className="divide-y-2 divide-gray-300">
                {selectedPackageMaterials.accessories.map((material, idx) => (
                  <tr key={material.id} className={`${idx % 2 === 0 ? 'bg-white' : 'bg-blue-50'} hover:bg-blue-100 transition-colors`}>
                    {editingMaterial?.id === material.id ? (
                      <>
                        <td className="py-2 px-4 border-r-2 border-gray-300">
                          <span className="font-medium text-gray-900">{material.variant_name}</span>
                        </td>
                        <td className="py-2 px-4 border-r-2 border-gray-300">
                          <input type="number" value={materialForm.quantity} onChange={(e) => setMaterialForm(f => ({ ...f, quantity: Number(e.target.value) }))} className="w-16 px-2 py-1 border border-gray-300 rounded text-sm text-center focus:ring-2 focus:ring-blue-500 focus:border-blue-500" min={1} />
                        </td>
                        <td className="py-2 px-4 border-r-2 border-gray-300">
                          <div className="flex gap-1 justify-center items-center">
                            <input type="number" placeholder="L" value={materialForm.length} onChange={(e) => setMaterialForm(f => ({ ...f, length: e.target.value }))} className="w-12 px-1 py-1 border border-gray-300 rounded text-sm text-center" />
                            <span className="text-gray-400">×</span>
                            <input type="number" placeholder="W" value={materialForm.width} onChange={(e) => setMaterialForm(f => ({ ...f, width: e.target.value }))} className="w-12 px-1 py-1 border border-gray-300 rounded text-sm text-center" />
                            <span className="text-gray-400">×</span>
                            <input type="number" placeholder="H" value={materialForm.height} onChange={(e) => setMaterialForm(f => ({ ...f, height: e.target.value }))} className="w-12 px-1 py-1 border border-gray-300 rounded text-sm text-center" />
                          </div>
                        </td>
                        <td className="py-2 px-4 text-center border-r-2 border-gray-300">
                          <label className="flex items-center justify-center gap-1.5 text-xs font-medium cursor-pointer select-none">
                            <input type="checkbox" checked={materialForm.is_final} onChange={(e) => setMaterialForm(f => ({ ...f, is_final: e.target.checked }))} className="rounded border-gray-300 text-blue-600 focus:ring-blue-500" />
                            Final
                          </label>
                        </td>
                        <td className="py-2 px-4 border-r-2 border-gray-300">
                          <input type="text" value={materialForm.comment} onChange={(e) => setMaterialForm(f => ({ ...f, comment: e.target.value }))} className="w-full px-2 py-1 border border-gray-300 rounded text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500" placeholder="Comment" />
                        </td>
                        <td className="py-2 px-4">
                          <div className="flex gap-2 justify-center">
                            <button onClick={handleUpdateMaterial} disabled={updatePackageMaterialMutation.isPending} className="p-1.5 text-white bg-green-600 hover:bg-green-700 rounded shadow-sm transition-colors">
                              {updatePackageMaterialMutation.isPending ? <Loader2 className="w-4 h-4 animate-spin" /> : <Check className="w-4 h-4" />}
                            </button>
                            <button onClick={() => setEditingMaterial(null)} className="p-1.5 text-gray-700 bg-gray-200 hover:bg-gray-300 rounded shadow-sm transition-colors">
                              <X className="w-4 h-4" />
                            </button>
                          </div>
                        </td>
                      ...
                      <>
                        <td className="py-3 px-4 border-r-2 border-gray-300">
                          <div className="flex flex-col">
                            <span className="font-semibold text-gray-900">{material.variant_name}</span>
                            {material.material_name && (
                              <span className="text-gray-500 text-xs">({material.material_name})</span>
                            )}
                          </div>
                        </td>
                        ...
                        <td className="py-3 px-4 text-gray-600 border-r-2 border-gray-300 max-w-xs truncate">
                          {material.comment || <span className="text-gray-400 italic">No comment</span>}
                        </td>
                        <td className="py-3 px-4">
                          <div className="flex gap-2 justify-center">
                            <button onClick={() => startEditMaterial(material)} className="p-1.5 text-blue-600 hover:bg-blue-50 rounded transition-colors">
                              <Edit className="w-4 h-4" />
                            </button>
                            <button onClick={() => handleDeleteMaterial(material.id)} disabled={deletePackageMaterialMutation.isPending} className="p-1.5 text-red-600 hover:bg-red-50 rounded transition-colors">
        ...
        <div className="flex flex-col items-center justify-center p-12 bg-gray-50 rounded-lg border-2 border-dashed border-gray-300">
          ...
          <p className="text-sm text-gray-400 mt-1">Click "Add Accessory" to get started</p>
        </div>
      )}
    </div>
  )
