...
export function ManufacturingTab({ selectedPackageManufacturing }: ManufacturingTabProps) {
  const [activeTab, setActiveTab] = useState<'big_sides' | 'small_sides' | 'lid' | 'base'>('big_sides')

  // Group manufacturing data by side and is_final
  const groupedData = selectedPackageManufacturing.reduce((acc, item) => {
    ...
    return acc
  }, {} as Record<string, { original: ManufacturingSecuringData | null; final: ManufacturingSecuringData | null }>)
      ...
      <div className="space-y-4">
        ...
        <div className="flex items-center justify-center p-12 bg-gray-50 rounded-lg border-2 border-dashed border-gray-300">
          <div className="text-center">
            ...
            <p className="text-gray-500 font-medium">No manufacturing data for this package</p>
          </div>
        </div>
      </div>
    )
    ...
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-lg border border-gray-200 shadow-sm overflow-hidden">
        <div className="flex border-b border-gray-200 overflow-x-auto">
          {(['big_sides', 'small_sides', 'lid', 'base'] as const).map((side) => (
            <button
              key={side}
