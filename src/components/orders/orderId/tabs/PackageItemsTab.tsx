...
import { Plus, Edit, Trash2, Check, X, Loader2, Camera } from 'lucide-react'
...
interface PackageItemsTabProps {
  ...
  updatePackageItemMutation: UseMutationResult<PackageItem, Error, {
    id: string
    designation?: string
    quantity?: number
    length?: number | null
              ...
              {editingItem?.id === item.id ? (
                <div className="w-full space-y-2">
                  <div className="flex gap-2">
                    ...
                    <input
                      ...
                      onChange={(e) => setItemForm(f => ({ ...f, designation: e.target.value }))}
                      className="flex-1 px-2 py-1 border rounded text-sm"
                      placeholder="Designation"
                    />
                  </div>
                  <div className="flex gap-2 items-center">
                    <input type="number" placeholder="L" value={itemForm.length} onChange={(e) => setItemForm(f => ({ ...f, length: e.target.value }))} className="w-16 px-2 py-1 border rounded text-sm" />
                    <span className="text-gray-400">×</span>
                    <input type="number" placeholder="W" value={itemForm.width} onChange={(e) => setItemForm(f => ({ ...f, width: e.target.value }))} className="w-16 px-2 py-1 border rounded text-sm" />
                    <span className="text-gray-400">×</span>
                    <input type="number" placeholder="H" value={itemForm.height} onChange={(e) => setItemForm(f => ({ ...f, height: e.target.value }))} className="w-16 px-2 py-1 border rounded text-sm" />
                    ...
                    <div className="flex gap-1 ml-auto">
                      ...
                      >
                        {updatePackageItemMutation.isPending ? <Loader2 className="w-4 h-4 animate-spin" /> : <Check className="w-4 h-4" />}
                      </button>
                      <button onClick={() => setEditingItem(null)} className="p-1.5 bg-gray-100 text-gray-700 rounded hover:bg-gray-200">
                        <X className="w-4 h-4" />
                      </button>
                    </div>
                  </div>
                ...
                <>
                  ...
                  <div className="flex items-center gap-2 self-end md:self-auto">
                    ...
                    <button onClick={() => startEditItem(item)} className="p-1.5 text-blue-600 hover:bg-blue-50 rounded">
                      <Edit size={16} />
                    </button>
                    <button
