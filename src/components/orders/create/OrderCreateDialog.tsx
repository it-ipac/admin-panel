...
export function OrderCreateDialog({
	...
	const {
		...
		const previews = rawPackages.map((pkg) => {
			...
			const buildTypePreview = (
				...
				if (
					...
					const manualMatch = scopedOptions.find(
						(v) =>
							(v.variant_name || "").toUpperCase() ===
							(label || "").toUpperCase(),
					);
					if (manualMatch) {
						console.log(
							`[Match Fail] FOUND EXACT CASE-INSENSITIVE MATCH: "${manualMatch.variant_name}"`,
						);
						console.log(
							`[Match Fail] DB Normalized: "${normalizeVariantName(manualMatch.variant_name)}"`,
						);
					}
				}
			...
			const accessoryPreviews = pkg.accessories.map(
				(part: RawPackageRow["accessories"][number], index: number) =>
					buildTypePreview(
						part.typeLabel,
		...
		const resolved = previews.map((preview) => ({
			...
			securing:
				(preview as any).securing?.map((part: any) => ({
					typeId: part.typeId,
					quantity: part.quantity,
					width: part.width,
					thickness: part.thickness,
			...
			accessories:
				(preview as any).accessories?.map((part: any) => ({
					typeId: part.typeId,
	...
	const detailTables: OrderCreateDetailTable[] = useMemo(() => {
		...
		tables.push({
			...
			description:
				"Packages created from the Calculation sheet (row 4 onward).",
			columns: [
				{
	...
	const validateForm = () => {
		...
		if (excelFile && packageCount === 0) {
			errors.file =
				"No package rows detected. Check column B starting at row 4.";
		}
	...
	const parsePackageRows = (sheet: ExcelJS.Worksheet) => {
		...
		const getText = (address: string) =>
			sheet.getCell(address).text?.trim() || "";
		...
		for (let row = 4; row < 1000; row += 1) {
			...
			for (let col = accessoryStart; col <= accessoryEnd; col += 1) {
				...
				const amount = parseNumberText(
					sheet.getCell(row, col).text?.trim() || "",
				);
	...
	const handleConfirmCreate = async () => {
		...
		try {
			...
			for (const pkg of resolvedPackages) {
				...
				const { data: finalInfo, error: finalError } =
					await db.createPackageInfo({});
				...
				const createSide = async (
					...
					const { error: finalSecuringError } =
						await db.createOrderPackageSecuring({
							order_package_id: orderPackage.id,
				...
				if (securingPayload.length > 0) {
					const { error: securingError } =
						await db.createOrderPackageMaterials(securingPayload);
					if (securingError) throw securingError;
				}
				...
				if (accessoryPayload.length > 0) {
					const { error: accessoryError } =
						await db.createOrderPackageMaterials(accessoryPayload);
							...
							<div>
								<div className="flex items-center justify-between mb-2">
									<p className="text-sm font-medium text-gray-900">
										normalizePackingTypeValue, normalizeVariantName,
										isGenericWood, isWoodVariantOption, nearlyEqual, Excel
										upload
									</p>
									{isParsing ? (
								...
								{packageCount > 0 && (
									<p className="mt-2 text-xs text-gray-600">
										Detected {packageCount} package row(s) in column B starting
										at row 4.
									</p>
								)}
								{hasUnresolvedMappings && (
						...
						<div className="flex justify-end gap-2 mt-6">
							...
							<button
								...
								className={`flex items-center gap-2 px-4 py-2 text-sm rounded-lg text-white transition-colors ${
									isParsing ? "bg-blue-300" : "bg-blue-600 hover:bg-blue-700"
								}`}
							>
								{isParsing ||
