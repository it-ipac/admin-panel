...
export const TwoTierCard: React.FC<TwoTierCardProps> = ({
  ...
  const hasChanged = final !== null && final !== undefined && original !== final;
  ...
  return (
    <div className={cn(`flex flex-col border rounded-xl p-1 m-1 min-w-[100px] flex-1 ${borderColor} ${headerBgColor}`, className)}>
      <div className="mb-1 px-2 rounded-full self-center">
        <span className={cn(`${headerTextColor} text-xs font-semibold text-center block`)}>
          {label}{showChangeHighlight ? ' *' : ''}
        </span>
      </div>

      {/* Original Row */}
      <div className="border border-indigo-200 rounded-lg px-1 py-1 mb-1 flex flex-col items-center justify-center bg-gray-50">
        <span className="text-[10px] text-amber-900 bg-amber-100 text-center w-full rounded-sm mb-1">Original</span>
        <span className="text-gray-900 text-sm font-semibold text-center w-full truncate px-1" title={formatValue(original)}>
      ...
      <div className={cn(
        ...
        showChangeHighlight ? 'border-orange-400 bg-orange-50' : 'border-indigo-200 bg-white'
      )}>
        <span className={cn(
          "text-[10px] text-center w-full rounded-sm mb-1",
          showChangeHighlight ? 'text-orange-900 bg-orange-100' : 'text-green-900 bg-green-100'
        )}>
          ...
          type === 'select' ? (
            ...
            <input
              ...
              onChange={(e) => {
                ...
                onChange?.(type === 'number' ? (val === '' ? null : Number(val)) : val);
              }}
            />
          )
        ) : (
