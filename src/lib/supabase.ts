...
export const db = {
  getProfile: async (userId: string) => {
    return supabase.from('profiles').select('*, roles(*)').eq('id', userId).single();
  },
  ...
  getMaterialVariants: async () => {
    return supabase
      ...
      .select('id, variant_name, width, thickness, unit:units_of_measure!material_variants_unit_id_fkey(id, name)')
  ...
  getMaterialVariantsByTag: async (tagName: string) => {
    return supabase
      .from('material_variants')
      .select('id, variant_name, width, thickness, material_variant_tags(tags(name))')
  ...
  createPackageItems: async (payload: Array<{
    order_package_id: string;
    quantity: number;
    designation: string;
    length?: number | null;
  ...
  createOrderPackageMaterials: async (payload: Array<{
    order_package_id: string;
  ...
  createSecuringTemplate: async (payload: {
    ...
    return supabase.from('securing_template').insert(payload).select('*').single();
  },
  ...
  createOrderPackageSecuring: async (payload: {
    ...
    return supabase.from('order_package_securing').insert(payload).select('*').single();
  },
  ...
  getOrderById: async (id: string) => {
    return supabase.from('orders').select('*, order_packages(*)').eq('id', id).single();
  },

  getUsers: async () => {
    return supabase.from('profiles').select('*, roles(*)').order('full_name', { ascending: true });
  },
  ...
  getInventory: async () => {
    return supabase.from('materials').select('*, material_variants(*)').order('name', { ascending: true });
  },

  getOrderStats: async () => {
    const { data: orders } = await supabase.from('orders').select('production_status, created_at');
